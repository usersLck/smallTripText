    ...
    if (self) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (type) {
                ...
                UIImageView * view2 = [[UIImageView alloc] initWithFrame:CGRectMake(label1.frame.origin.x + (label1.bounds.size.width - 32) / 2 , label1.frame.origin.y + label1.frame.size.height, label1.bounds.size.height, label1.bounds.size.height)];
                [self addSubview:view2];
